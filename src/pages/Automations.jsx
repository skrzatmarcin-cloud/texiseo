...
export default function Automations() {
  ...
  return (
    <div className="p-4 lg:p-6 max-w-[1100px] mx-auto">
      <PageHeader title="Automatyzacje" description="Inteligentne wyzwalacze przepływu pracy i kontrolki automatyzacji systemu Linguatoons" />
            ...
            <div key={autom.id} className="bg-card border border-border rounded-xl overflow-hidden">
              <div className="flex items-center justify-between gap-4 px-5 py-4">
                  ...
                  <div className="min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {autom.isEntityTriggered && (
                        <span className="text-[10px] bg-secondary text-muted-foreground px-1.5 py-0.5 rounded">Auto-wyzwalane</span>
                      )}
                    </div>
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  ...
                  {autom.isUIOnly ? (
                    <a href={autom.uiLink} className="text-xs text-primary hover:underline font-medium">Otwórz →</a>
                  ) : (
                    <Button size="sm" variant="outline" className="h-7 text-xs gap-1"
                      onClick={() => setExpanded(p => ({ ...p, [autom.id]: !isOpen }))}>
                      {isOpen ? <ChevronUp className="h-3 w-3" /> : <ChevronDown className="h-3 w-3" />}
                      {isOpen ? "Zamknij" : "Skonfiguruj"}
                    </Button>
                  )}
                </div>
              </div>
              ...
              {isOpen && (
                <div className="border-t border-border px-5 py-4 bg-secondary/20">
                  ...
                  <p className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wide mb-2">Działanie</p>
                  ...
                  <div className="flex items-center gap-2">
                    <Button size="sm" className="gap-1.5 h-8" onClick={() => runAutomation(autom)} disabled={state === "running"}>
                      ...
                      {state === "running" ? "Uruchamianie…" : "Uruchom teraz"}
                    </Button>
                    {result && (
      ...
      <div className="mt-4 bg-primary/5 border border-primary/20 rounded-xl p-4">
        <div className="flex items-center gap-2 mb-1">
          <Zap className="h-4 w-4 text-primary" />
          <p className="text-xs font-semibold text-primary">Aktywna automatyzacja encji</p>
        </div>
        <p className="text-[11px] text-muted-foreground">Przepływ <strong>Zatwierdzenie → Brief</strong> jest również podpięty jako wyzwalacz bazy danych: any Content Idea moved to <code className="bg-secondary px-1 rounded">approved</code> status will automatically queue for brief generation. Configure the scheduled automation from the platform's Automations panel.</p>
      </div>
    </div>
  );
}