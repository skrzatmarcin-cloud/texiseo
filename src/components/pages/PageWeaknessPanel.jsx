...
const SEVERITY_CONFIG = {
  critical: { icon: AlertCircle, color: "text-red-600", bg: "bg-red-50", ring: "ring-red-200", label: "Critical" },
  high: { icon: AlertTriangle, color: "text-amber-600", bg: "bg-amber-50", ring: "ring-amber-200", label: "High" },
  medium: { icon: Info, color: "text-blue-600", bg: "bg-blue-50", ring: "ring-blue-200", label: "Medium" },
  low: { icon: Minus, color: "text-slate-500", bg: "bg-slate-50", ring: "ring-slate-200", label: "Low" },
};
    ...
    return (
      <div className="bg-card rounded-xl border border-emerald-200 p-5">
        <div className="flex items-center gap-2 text-emerald-600">
          ...
          <h3 className="text-sm font-semibold">No Critical Weaknesses Detected</h3>
        </div>
        <p className="text-xs text-muted-foreground mt-2">This page passes all automated health checks. Continue monitoring for content decay and internal link quality.</p>
      </div>
    );
  }
  ...
  return (
    <div className="bg-card rounded-xl border border-border p-5">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-sm font-semibold">Weakness Summary</h3>
        <span className="text-[11px] text-muted-foreground">{weaknesses.length} issue{weaknesses.length !== 1 ? "s" : ""} found</span>
      </div>
